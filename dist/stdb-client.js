...
${originalIndentation}`;
  ...
  // src/stdb-client.ts
  var stdb_client_exports = {};
  __export(stdb_client_exports, {
    addPost: () => addPost,
    connect: () => connect,
...
${ty.variants.map(
  ...
  function tablesToSchema(ctx, tables3) {
    return {
      tables: Object.fromEntries(
        Object.entries(tables3).map(([accName, schema2]) => [
          accName,
  ...
  function schema(tables3) {
    const ctx = new ModuleContext();
    return new Tables(tablesToSchema(ctx, tables3));
  }
  ...
  // src/module_bindings/add_blog_post_reducer.ts
  var add_blog_post_reducer_default = {
    title: t.string(),
    category: t.string(),
    date: t.string(),
    summary: t.string(),
  ...
  // src/module_bindings/delete_blog_post_reducer.ts
  var delete_blog_post_reducer_default = {
    id: t.u64()
  };

  // src/module_bindings/update_blog_post_reducer.ts
  var update_blog_post_reducer_default = {
    id: t.u64(),
    title: t.string(),
    category: t.string(),
    date: t.string(),
  ...
  // src/module_bindings/blog_post_table.ts
  var blog_post_table_default = t.row({
    id: t.u64().primaryKey(),
    title: t.string(),
    category: t.string(),
    date: t.string(),
  ...
  var tablesSchema = schema({
    blog_post: table({
      ...
      constraints: [
        { name: "blog_post_id_key", constraint: "unique", columns: ["id"] }
      ]
    }, blog_post_table_default)
  });
  var reducersSchema = reducers(
    reducerSchema("add_blog_post", add_blog_post_reducer_default),
    reducerSchema("delete_blog_post", delete_blog_post_reducer_default),
    reducerSchema("update_blog_post", update_blog_post_reducer_default)
  );
  ...
  // src/stdb-client.ts
  var SPACETIMEDB_URI = "wss://maincloud.spacetimedb.com";
  var DATABASE_NAME = "server-fpea2";
  var conn = null;
  var blogPostsCache = [];
  var onPostsUpdated = null;
  function refreshPostsFromCache(ctx) {
    const posts = [];
  ...
  function buildConnection(callback, token) {
    conn = DbConnection.builder().withUri(SPACETIMEDB_URI).withDatabaseName(DATABASE_NAME).withToken(token).onConnect((ctx, identity, newToken) => {
      console.log("[SpaceTimeDB] Connected as", identity.toHexString());
      localStorage.setItem("stdb_portfolio_token", newToken);
      conn.subscriptionBuilder().onApplied((subCtx) => {
      ...
      if (!hasRetried && localStorage.getItem("stdb_portfolio_token")) {
        ...
        conn = null;
        buildConnection(callback, void 0);
      }
    }).build();
  ...
  function connect(callback) {
    ...
    hasRetried = false;
    const token = localStorage.getItem("stdb_portfolio_token") || void 0;
    buildConnection(callback, token);
  }
  function getPosts() {
    return blogPostsCache;
  }
  function addPost(title, category, date, summary, body) {
    if (!conn) return;
    conn.reducers.addBlogPost({ title, category, date, summary, body });
  }
  function updatePost(id, title, category, date, summary, body) {
    if (!conn) return;
  ...
  return __toCommonJS(stdb_client_exports);
})();
